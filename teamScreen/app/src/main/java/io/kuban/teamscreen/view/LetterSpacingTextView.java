...
public class LetterSpacingTextView extends TextView {
    ...
    //设置高亮突出显示需在setText之前设置
    public void setHighlighted(String highlighted, int highlightedColor) {
...
//            }
//            ForegroundColorSpan colorSpan = new ForegroundColorSpan(highlightedColor);
//            finalText.setSpan(colorSpan, 2, builder.length(), Spanned.SPAN_INCLUSIVE_EXCLUSIVE);
            if (!TextUtils.isEmpty(highlighted)) {
                List<Integer> start = StringUtil.searchAllIndex(builder.toString(), highlighted);
                for (Integer integer : start) {
//                    if (integer  > 0) {
                    ForegroundColorSpan colorSpan = new ForegroundColorSpan(highlightedColor);
                    finalText.setSpan(colorSpan, integer, (integer + 1), Spanned.SPAN_INCLUSIVE_EXCLUSIVE);
//                    }
                }
            }
        }
